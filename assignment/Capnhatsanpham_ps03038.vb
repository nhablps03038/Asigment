...
Public Class Capnhatsanpham_ps03038
    ...
    Private Sub btnLoad_Click(sender As Object, e As EventArgs) Handles btnLoad.Click
        ...
        connect.Open()
        Dim xem As SqlDataAdapter = New SqlDataAdapter("select SANPHAM.MASP as 'Mã sản phẩm',SANPHAM.TENSP as 'Tên sản phẩm', LOAI_SANPHAM.MALOAI as 'Mã Loại', LOAI_SANPHAM.TENLOAI as 'Tên Loại',SANPHAM.SOLUONG as 'Số lượng' from SANPHAM inner join LOAI_SANPHAM on SANPHAM.MASP = LOAI_SANPHAM.MASP where SANPHAM.MASP='" & txtMASP.Text & "'", connect)
        Try
    ...
    Private Sub LoadData()
        Dim conn As SqlConnection = New SqlConnection(chuoiketnoi)
        Dim load As SqlDataAdapter = New SqlDataAdapter("select SANPHAM.MASP as 'Mã sản phẩm',SANPHAM.TENSP as 'Tên sản phẩm', LOAI_SANPHAM.MALOAI as 'Mã Loại', LOAI_SANPHAM.TENLOAI as 'Tên Loại',SANPHAM.SOLUONG as 'Số lượng' from SANPHAM inner join LOAI_SANPHAM on SANPHAM.MASP = LOAI_SANPHAM.MASP", conn)
    ...
    Private Sub btnCapnhat_Click(sender As Object, e As EventArgs) Handles btnCapnhat.Click
        Dim conn As SqlConnection = New SqlConnection(chuoiketnoi)
        Dim updatequery As String = "update SANPHAM set MASP=@MASP, TENSP=@TENSP, SOLUONG=@SOLUONG where MASP=@MASP update LOAI_SANPHAM set MALOAI=@MALOAI, MASP=@MASP, TENLOAI=@TENLOAI where MASP=@MASP"
    ...
    Private Sub btnXoa_Click(sender As Object, e As EventArgs) Handles btnXoa.Click
        Dim delquery As String = "delete from SANPHAM where MASP=@MASP delete from LOAI_SANPHAM where MASP=@MASP"
    ...
    Private Sub btnThem_Click(sender As Object, e As EventArgs) Handles btnThem.Click
        Dim conn As SqlConnection = New SqlConnection(chuoiketnoi)
        Dim query As String = "insert into SANPHAM values(@MASP,@TENSP,@SOLUONG) insert into LOAI_SANPHAM values(@MALOAI,@MASP,@TENLOAI)"
        ...
        Dim refesh As SqlDataAdapter = New SqlDataAdapter("select SANPHAM.MASP as 'Mã sản phẩm',SANPHAM.TENSP as 'Tên sản phẩm', LOAI_SANPHAM.MALOAI as 'Mã Loại', LOAI_SANPHAM.TENLOAI as 'Tên Loại',SANPHAM.SOLUONG as 'Số lượng' from SANPHAM inner join LOAI_SANPHAM on SANPHAM.MASP = LOAI_SANPHAM.MASP", conn)
        db.Clear()
        refesh.Fill(db)
        dgvSanpham.DataSource = db.DefaultView
    End Sub
